    ...
    def import_excel(self):
        ...
        for file_path in file_paths:
            if "Case Inquiry" in file_path:
                try:
                    ...
                    df = df.drop_duplicates(subset=['Case'])  # Remove duplicate cases based on the 'Case' column
                    
                    # Ensure the second column header is "Inbound Shipment"
                    if df.columns[1] != "Inbound Shipment":
                    ...
                    df.dropna(subset=["Inbound Shipment"], inplace=True)  # Remove rows where 'Inbound Shipment' is None
        ...
        if Inbound_Shipment_inquiry_data:
            ...
            for i in range(len(Inbound_Shipment_inquiry_data['Inbound Shipment'])):
                self.instructions_text.insert(tk.END, f"{Inbound_Shipment_inquiry_data['Inbound Shipment'][i]}, {Inbound_Shipment_inquiry_data['Cases Shipped'][i]}, {Inbound_Shipment_inquiry_data['Cases Received'][i]}, {Inbound_Shipment_inquiry_data['Matched Case Counts'][i]}\n")
