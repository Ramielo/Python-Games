...
class RSV_1(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("[AHK coding] Receiving")
...
class Empty_CC_1(PredefinedWindow):
    def __init__(self, parent, window_size):
        super().__init__(parent, "[AHK coding] Cycle Count Empty", window_size)
...
class Empty_CC_2(PredefinedWindow):
    ...
    def is_notepad_plus_plus_active(self):
        """
        检查Notepad++窗口是否最上层和未关闭。
        返回True如果Notepad++是活动的且最上层，否则返回False。
        """
        try:
    ...
    def export_to_txt(self):
        def activate_notepad_plus_plus():
            try:
                ...
                message = "\n\nNotepad++ window not found. Please make sure Notepad++ is open."
        ...
        if not activate_notepad_plus_plus():
            return  # If Notepad++ cannot be activated, just return
        ...
        for index in range(self.current_location_index, len(locations)):
            if not is_notepad_plus_plus_active():
                message = f"\n\nNotepad++ window is no longer on top or has been closed. Execution paused at Location: {locations[index]}"
                self.text_box.insert(tk.END, message)
                self.text_box.see(tk.END)  # Scroll to the bottom
